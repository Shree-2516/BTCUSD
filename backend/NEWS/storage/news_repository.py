...
class NewsRepository:
    ...
    @classmethod
    def get_sentiment_trends(cls, days=7):
        """Fetches market sentiment signals over the last N days."""
        db = cls.get_db()
        since = datetime.utcnow() - timedelta(days=days)
        try:
            return db.query(MarketSentimentModel).filter(MarketSentimentModel.created_at >= since).order_by(MarketSentimentModel.created_at).all()
        except Exception as e:
            logger.error(f"Database error fetching sentiment trends: {e}")
            return []
        finally:
            db.close()

    @classmethod
    def get_high_impact_events(cls, confidence_threshold=85, limit=20):
        """Fetches news articles with high confidence scores."""
        db = cls.get_db()
        try:
            return db.query(NewsArticle).filter(NewsArticle.confidence >= confidence_threshold).order_by(desc(NewsArticle.published_at)).limit(limit).all()
        except Exception as e:
            logger.error(f"Database error fetching high impact events: {e}")
            return []
        finally:
            db.close()
            
    @classmethod
    def save_news_summary(cls, topics_json):
        """Saves an AI generated news summary."""
        db = cls.get_db()
        try:
            summary = NewsSummaryModel(topics=topics_json)
            db.add(summary)
            db.commit()
            db.refresh(summary)
            return summary
            ...
            db.rollback()
            logger.error(f"Database error saving news summary: {e}")
            raise e
        finally:
            db.close()
            
    @classmethod
    def get_latest_summary(cls):
        """Fetches the latest AI news summary."""
        db = cls.get_db()
        try:
            return db.query(NewsSummaryModel).order_by(desc(NewsSummaryModel.created_at)).first()
        except Exception as e:
            logger.error(f"Database error fetching latest summary: {e}")
            return None
        finally:
            db.close()
