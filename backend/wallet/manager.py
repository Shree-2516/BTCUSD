from backend.database.db import SessionLocal, Wallet, Transaction, WalletTransaction
...
class WalletManager:
    ...
    def get_wallet_transactions(self, limit=100, tx_type=None):
        db = self.get_session()
        try:
            query = db.query(WalletTransaction)
            if tx_type and tx_type != "ALL":
                query = query.filter(WalletTransaction.type == tx_type)
            transactions = query.order_by(desc(WalletTransaction.timestamp)).limit(limit).all()
            return [
                {
                    "id": t.id,
                    "timestamp": t.timestamp.strftime("%Y-%m-%d %H:%M:%S"),
                    ...
                    "used_margin_after": t.used_margin_after,
                    "reason": t.reason,
                    "trade_id": t.trade_id,
                }
                for t in transactions
